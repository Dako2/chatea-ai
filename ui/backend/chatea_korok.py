from backend.vecDB import VecDataBase
import openai
import json
from backend.llm.llm_agent import LLM
from backend.llm.llm_tools import price_from_token_num
import os
...
class Korok:
    ...
    def chat_api_v2(self, user_input):
        ...
        output, usage = self.convo.rolling_chat(user_input, local_db_text)
        cost = price_from_token_num(usage.prompt_tokens, usage.completion_tokens)
        
        return output, cost


def recommend(user_input):
    recommendation, cost = korok.chat_api_v2(user_input)
