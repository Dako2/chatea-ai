...
NAME_EMBEDDING_MODEL = 'distiluse-base-multilingual-cased-v1'
#all-MiniLM-L6-v2
...
class VecDataBase():
    ...
    def load_db(self, db_paths):
        for db_json_file in db_paths:
            print(f"processing {db_json_file}")
            if db_json_file not in list(self.cache_vector_database.keys()): #quick load corpus_json
                try:
            ...
            if self.update_db or not os.path.exists(db_ebd_file): #update the embeddings
                self.convert_json_to_embeddings(db_json_file) #convert the embeddings
...
if __name__ == "__main__":
    ...
    while True:
        ...
        #result1, score1 = v.search_db(user_input, './datasets/tea.json', threshold=0.6, top_n = 1)
