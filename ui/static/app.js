...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('tea-form').addEventListener('submit', function(event) {
        ...
        // Add the 'button-clicked' class to change the button's appearance
        submitButton.classList.add('button-clicked'); // Add the class to change the button's color

        var symptoms = document.getElementById('symptoms').value;
        var processingMessage = document.getElementById('processing-message'); // Add this element to your HTML
    

        // Display a processing message
        processingMessage.textContent = 'Processing your submission, please wait...';
        processingMessage.style.display = 'block'; // Make sure this is visible
        ...
        .then(data => {
            ...
            // Remove the 'button-clicked' class and hide the processing message
            submitButton.classList.remove('button-clicked');
            processingMessage.style.display = 'none'; // Hide the message
        })
        .catch((error) => {
            console.error('Error:', error);
            document.getElementById('result').textContent = `Error: ${error.message}`;
    
            // Remove the 'button-clicked' class and update/hide the processing message
            submitButton.classList.remove('button-clicked');
            processingMessage.textContent = 'Error occurred, please try again.';
            setTimeout(() => processingMessage.style.display = 'none', 3000); // Hide after 3 seconds
        });
    });
